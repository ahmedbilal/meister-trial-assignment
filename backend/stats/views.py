...
from rest_framework.generics import GenericAPIView, CreateAPIView, ListAPIView, RetrieveUpdateAPIView, ListCreateAPIView
from .serializers import LoginSerializer, CountrySerializer, UserSerializer, SalesSerializer
...
class StatisticsView(GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        user_aggregate = get_user_model().objects.get(id=3).sales.aggregate(total_revenue=Sum("revenue"), total_sales=Count("id"), highest_revenue=Max("revenue"))
        all_user_aggregate = get_user_model().objects.all().aggregate(total_revenue=Sum("sales__revenue"), total_sales=Count("sales__id"))

        highest_revenue_sale_for_current_user = get_user_model().objects.get(id=3).sales.filter(revenue=user_aggregate["highest_revenue"]).first()

        average_sales_for_current_user = user_aggregate['total_revenue'] / user_aggregate["total_sales"]
        average_sales_for_all_user = all_user_aggregate['total_revenue'] / all_user_aggregate['total_sales']
        return JsonResponse({
